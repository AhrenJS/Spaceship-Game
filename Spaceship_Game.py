...
Bullet_Shoot_Sound = pygame.mixer.Sound(os.path.join("Assets", "Gun-Silencer.ogg"))
main_theme = pygame.mixer.Sound(os.path.join("Assets", "Imperial-March.ogg"))
victory_scream = pygame.mixer.Sound(os.path.join("Assets", "Wilhelm-Scream.ogg"))
...
def draw_window(yellow_pos, red_pos, yellow_bullet, red_bullet, yellow_health, red_health, yellow_score, red_score):
    ...
    draw_score_yellow = Score_Font.render("Yellow : " + str(len(yellow_score)), True, WHITE)
    WIN.blit(draw_score_yellow, (10, HEIGHT - draw_score_yellow.get_height()))
    draw_score_red = Score_Font.render("Red : " + str(len(red_score)), True, WHITE)
...
def maingame():
    ...
    while run:
        ...
        if red_health <= 0:
            winner = "Yellow Wins!"
            yellow_score.append("Win")
            win_image += 1
        if yellow_health <= 0:
            winner = "Red Wins!"
            red_score.append("Win")
            win_image += 2
        if winner != "":
